...
HI_S32 ISP_DRV_LSCRegConfig(ISP_REG_CFG_S *pstRegCfgInfo)
{
    HI_S32 i;

    if (pstRegCfgInfo->stKernelCfg.unKey.bit1LscCfg)
	{
	    /* write horizontal grid info */
        for (i = 0; i < 8; i++)
	    {
	        hi_isp_lsc_hwin_info_write(i, (2*(pstRegCfgInfo->stKernelCfg.stLscRegCfg.xgrid[i]) - 1), pstRegCfgInfo->stKernelCfg.stLscRegCfg.xinvgrid[i]);
	    }

	    /* write vertical grid info */
	    for (i = 0; i < 8; i++)
	    {
	        hi_isp_lsc_vwin_info_write(i, (2*(pstRegCfgInfo->stKernelCfg.stLscRegCfg.ygrid[i]) - 1), pstRegCfgInfo->stKernelCfg.stLscRegCfg.yinvgrid[i]);
	    }
		...
		for (i = 0; i < 289; i++)
		{
		    hi_isp_lsc_grr_gain_wdata_write(pstRegCfgInfo->stKernelCfg.stLscRegCfg.grr_gain[i]);
		}

		hi_isp_lsc_gbb_gain_waddr_write(0);
		for (i = 0; i < 289; i++)
		{
		    hi_isp_lsc_gbb_gain_wdata_write(pstRegCfgInfo->stKernelCfg.stLscRegCfg.gbb_gain[i]);
		}
	
		pstRegCfgInfo->stKernelCfg.unKey.bit1LscCfg = 0;
	}
...
HI_S32 ISP_DRV_SwitchMode(ISP_DRV_CTX_S *pstDrvCtx)
{
    ...
    {      
        ...
        hi_isp_drc_exposure_write(pstSyncCfg->u32DRCComp[0]);
...
static long ISP_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
    ...
    switch (cmd)
    {
        ...
        { 
            ...
            spin_lock_irqsave(&g_stIspLock, u32Flags);
            memcpy(&stActStat, g_astIspDrvCtx[IspDev].stStatisticsBuf.pstActStat, sizeof(ISP_STAT_INFO_S));
            spin_unlock_irqrestore(&g_stIspLock, u32Flags);
            
			copy_to_user(argp, &stActStat, sizeof(ISP_STAT_INFO_S));
	    ...
	    case ISP_LSC_UPDATE_MODE_GET:
        {                      
			HI_U32 __user *argp = (HI_U32 __user *)arg;
			
            if (copy_to_user(argp, &lsc_update_mode, sizeof(HI_U32)))
            {
                printk(KERN_INFO "copy status to user failed!\n");
                return -EFAULT;
            }   
...
void ISP_IntBottomHalf(unsigned long data)
{
    ...
    /*LSC_Config*/
    if ((u32IspIntStatus & 0x40)&&(lsc_update_mode == 1))
    {
       ISP_DRV_LSCRegConfig(&pstDrvCtx->stRegCfgInfo[pstDrvCtx->u32RegCfgInfoFlag]);
    }
...
static int ISP_ProcShow(struct seq_file *s, void *pArg)
{
	...
	seq_printf(s, " %15s" " %15s" " %15s" " %15s"  " %15s\n",  "proc_param",  "bottomhalf", "update_pos", "pwm_num", "lsc_update_mode");
	seq_printf(s, " %15u" " %15u" " %15u" " %15u"  " %15u\n",   proc_param, int_bottomhalf,  update_pos,   pwm_num,   lsc_update_mode);
...
{
	if (0 > pwm_num || 3 < pwm_num)
	  {
		  printk("-----ModuleParam: pwm_num %d is illegal,should be [0,3], it will be set to 1!\n",pwm_num);
		  pwm_num = 1;
	  }
	if (0 > proc_param )
	  {
		  printk("-----ModuleParam: proc_param %d is illegal,should be [0, +¡Þ], it will be set to 0!\n",proc_param);
		  proc_param = 0;
	  }
	if (0 > update_pos  || 1 < update_pos)
	  {
		  printk("-----ModuleParam: update_pos %d is illegal,should be [0, 1], it will be set to 0!\n",update_pos);
		  update_pos = 0;
	  }
    if (0 > lsc_update_mode  || 1 < lsc_update_mode)
	  {
		  printk("-----ModuleParam: lsc_update_mode %d is illegal,should be [0, 1], it will be set to 0!\n",lsc_update_mode);
		  lsc_update_mode = 0;
	  }
}
...
module_param(lsc_update_mode, uint, S_IRUGO);
