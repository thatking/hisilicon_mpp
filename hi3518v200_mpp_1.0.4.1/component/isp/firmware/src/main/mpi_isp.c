...
HI_S32 HI_MPI_ISP_SetPubAttr(ISP_DEV IspDev, const ISP_PUB_ATTR_S *pstPubAttr)
{
    ...
    if ((pstPubAttr->stWndRect.u32Height < 240) || (pstPubAttr->stWndRect.u32Height > 2048))
    {
        ISP_TRACE(HI_DBG_ERR, "Invalid Image Height:%d!\n", pstPubAttr->stWndRect.u32Height);
        return HI_ERR_ISP_ILLEGAL_PARAM;
    }
    if ((pstPubAttr->stWndRect.s32X < 0) || (pstPubAttr->stWndRect.s32X > 2048 - 480))
    {
        ISP_TRACE(HI_DBG_ERR, "Invalid Image X:%d!\n", pstPubAttr->stWndRect.s32X);
        return HI_ERR_ISP_ILLEGAL_PARAM;
    }
    if ((pstPubAttr->stWndRect.s32Y < 0) || (pstPubAttr->stWndRect.s32Y > 2048 - 240))
    {
...
HI_S32 HI_MPI_ISP_SetDRCAttr(ISP_DEV IspDev, const ISP_DRC_ATTR_S *pstDRC)
{
    ...
    for (i = 0; i < COLORCORRECTIONLUT_NODE_NUMBER; i++)
    {    
	    if (pstDRC->au16ColorCorrectionLut[i] > 0xFFF) 
	    	{
    ...
    switch (pstDRC->enOpType)
    {
        case OP_TYPE_AUTO:
            ...
            if (pstDRC->stAuto.u8LocalMixingBright >0x80)
            {
                printf("Invalid u8LocalMixingBright Parameter Input!\n");
                return HI_ERR_ISP_ILLEGAL_PARAM;
            }
            if (pstDRC->stAuto.u8LocalMixingDark > 0x60)
            {
            ...
            hi_ext_system_drc_auto_localmixingbright_write(pstDRC->stAuto.u8LocalMixingBright);            
        ...
        case OP_TYPE_MANUAL:
            ...
            if (pstDRC->stManual.u8LocalMixingBright >0x80)
            {
                printf("Invalid u8LocalMixingBright Parameter Input!\n");
                return HI_ERR_ISP_ILLEGAL_PARAM;
            }
            if (pstDRC->stManual.u8LocalMixingDark > 0x60)
            {
                printf("Invalid u8LocalMixingDark Parameter Input!\n");
                return HI_ERR_ISP_ILLEGAL_PARAM;
            }
            ...
            hi_ext_system_drc_manual_localmixingbright_write(pstDRC->stManual.u8LocalMixingBright);            
	...
	hi_ext_system_drc_detail_mixing_thres_write(pstDRC->u8LocalMixingThres);

	//hi_isp_drc_var_spatial_write(pstDRC->u8SpatialVar);
	hi_ext_system_drc_var_spatial_write(pstDRC->u8SpatialVar);
    
	//hi_isp_drc_var_range_write(pstDRC->u8RangeVar);
	hi_ext_system_drc_var_range_write(pstDRC->u8RangeVar);
...
{  
    ...
    pstDRC->stAuto.u8LocalMixingBright = hi_ext_system_drc_auto_localmixingbright_read();
    ...
    pstDRC->stManual.u8LocalMixingBright = hi_ext_system_drc_manual_localmixingbright_read();
	...
	pstDRC->u8LocalMixingThres = hi_ext_system_drc_detail_mixing_thres_read();
	pstDRC->u8RangeVar = hi_ext_system_drc_var_range_read();
	pstDRC->u8SpatialVar = hi_ext_system_drc_var_spatial_read();
	
	hi_isp_drc_stat_ind_raddr2_write(0);

	for (i = 0; i < COLORCORRECTIONLUT_NODE_NUMBER; i++)
	{    
	   pstDRC->au16ColorCorrectionLut[i] = hi_isp_drc_stat_ind_rdata2_read(); 
	}
...
HI_S32 HI_MPI_ISP_GetStatistics(ISP_DEV IspDev, ISP_STATISTICS_S *pstStat)
{
    ...
    stActStatInfo.pVirtAddr = HI_MPI_SYS_Mmap(stActStatInfo.u32PhyAddr, sizeof(ISP_STAT_S));
    ...
    for (i = 0; i < 256; i++)
    {
        pstStat->stAEStat.au32Hist256Value[i] = pstIspActStat->stAeStat3.au32HistogramMemArray[i];
    }
    ...
    for(i = 0;i < AE_ZONE_ROW ; i++)
    {
        ...
        {           
            pstStat->stAEStat.au16ZoneAvg[i][j][0] = pstIspActStat->stAeStat5.au16ZoneAvg[i][j][0];  /*R */
            pstStat->stAEStat.au16ZoneAvg[i][j][1] = pstIspActStat->stAeStat5.au16ZoneAvg[i][j][1];  /*Gr*/
            pstStat->stAEStat.au16ZoneAvg[i][j][2] = pstIspActStat->stAeStat5.au16ZoneAvg[i][j][2];  /*Gb*/
            pstStat->stAEStat.au16ZoneAvg[i][j][3] = pstIspActStat->stAeStat5.au16ZoneAvg[i][j][3];  /*B */
        }       
    }
    ...
    for(i = 0; i < AWB_ZONE_ROW; i++)
    {
        ...
        {        
            pstStat->stWBStat.stBayerStatistics.au16ZoneAvgR[i][j] = pstIspActStat->stAwbStat4.au16MeteringMemArrayAvgR[i*AWB_ZONE_COLUMN+j];
            pstStat->stWBStat.stBayerStatistics.au16ZoneAvgG[i][j] = pstIspActStat->stAwbStat4.au16MeteringMemArrayAvgG[i*AWB_ZONE_COLUMN+j];
            pstStat->stWBStat.stBayerStatistics.au16ZoneAvgB[i][j] = pstIspActStat->stAwbStat4.au16MeteringMemArrayAvgB[i*AWB_ZONE_COLUMN+j];
            pstStat->stWBStat.stBayerStatistics.au16ZoneCountAll[i][j] = pstIspActStat->stAwbStat4.au16MeteringMemArrayCountAll[i*AWB_ZONE_COLUMN+j];
            pstStat->stWBStat.stBayerStatistics.au16ZoneCountMin[i][j] = pstIspActStat->stAwbStat4.au16MeteringMemArrayCountMin[i*AWB_ZONE_COLUMN+j];
            pstStat->stWBStat.stBayerStatistics.au16ZoneCountMax[i][j] = pstIspActStat->stAwbStat4.au16MeteringMemArrayCountMax[i*AWB_ZONE_COLUMN+j];
        }    
    }
