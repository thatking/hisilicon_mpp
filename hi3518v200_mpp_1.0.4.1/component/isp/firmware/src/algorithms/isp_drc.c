...
#define DRC_STRLENGTH_Bias          (0x80)
...
static HI_VOID DrcExtRegsDefault(HI_VOID)
{
    ...
    hi_ext_system_drc_manual_localmixingbright_write(DETAIL_MIXING_BRIGHT);
    hi_ext_system_drc_manual_localmixingdark_write(DETAIL_MIXING_DARK);
    hi_ext_system_drc_auto_localmixingbright_write(DETAIL_MIXING_BRIGHT);
    hi_ext_system_drc_auto_localmixingdark_write(DETAIL_MIXING_DARK);
    hi_ext_system_drc_detail_mixing_thres_write(DETAIL_MIXING_THRES);
    hi_ext_system_drc_var_spatial_write(VARSPATIAL);
    hi_ext_system_drc_var_range_write(VARRANGE);
    hi_ext_system_drc_enable_write(HI_FALSE);
    hi_ext_system_drc_auto_strength_write(DRC_STRLENGTH_Bias);
...
{	
	...
	if (IS_BUILT_IN_WDR_MODE(u8WDRMode))
	{
        ...
        hi_ext_system_drc_actual_strength_write(DRC_STRLENGTH_TARGET_WDR);
	}
	else
	{
        ...
        hi_ext_system_drc_actual_strength_write(DRC_STRLENGTH_TARGET_LINEAR);
	}
...
{   
    ...
    pstDrc->G2            = DRC_G2;
    pstDrc->K1            = DRC_K1;
    pstDrc->u32FrameCnt   = 0;
    

    pstDrc->bEnable               = pstSnsDft->stDrc.bEnable;
    //pstDrc->bDrcDitherEn          = (pstDrc->bEnable) ? HI_FALSE : HI_TRUE;
    //pstDrc->u8LocalMixingBright  = pstSnsDft->stDrc.u8LocalMixingBright;
    ...
    //hi_isp_drc_detail_mixing_bright_write(pstDrc->u8LocalMixingBright);
...
HI_S32 DrcCalStrength(ISP_DRC_S *pstDrc, HI_U32 u32Iso, HI_U8 u8SnsWDRMode, ISP_STAT_S *pStatInfo)
{
    ...
    pstDrc->u8LocalMixingBright = (HI_U8)((53 * u32Strength + 146) / 255);
    pstDrc->u8LocalMixingDark = (HI_U8)((21784 - 53 * u32Strength) / 255);
    
    pstDrc->u32Strength = u32Strength;
    pstDrc->u8LocalMixingBright = DRC_CLIP3(32, 64, pstDrc->u8LocalMixingBright);
    ...
    HI_VOID *pRegCfg, HI_S32 s32Rsv)
{
	...
	if ((0 == hi_isp_drc_enable_read())&&(0 == hi_ext_system_drc_enable_read()))
	{
        pstDrc->u32FrameCnt = 0;
        return HI_SUCCESS;   
	}
	...
	{   
		...
		hi_ext_system_drc_auto_localmixingbright_write(pstDrc->u8LocalMixingBright);
	...
	//for manual	
	{
        ...
        pstDrc->u8LocalMixingBright = hi_ext_system_drc_manual_localmixingbright_read();
        pstDrc->u8LocalMixingDark = hi_ext_system_drc_manual_localmixingdark_read();
	}
       hi_ext_system_drc_actual_strength_write(pstDrc->u32Strength);
       hi_isp_drc_detail_mixing_bright_write(pstDrc->u8LocalMixingBright);
       hi_isp_drc_detail_mixing_dark_write(pstDrc->u8LocalMixingDark);
