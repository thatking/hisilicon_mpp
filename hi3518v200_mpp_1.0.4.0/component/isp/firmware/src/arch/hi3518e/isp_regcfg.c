...
HI_S32 ISP_RegConfig(ISP_REG_CFG_S *pstRegCfgInfo)
{
    ...
    s32Ret = ioctl(g_as32IspFd[0], ISP_LSC_UPDATE_MODE_GET, &u32LscUpdateMode);

    if (HI_SUCCESS != s32Ret)
    {
        printf("Get u32LscUpdateMode failed %x!\n", s32Ret);
        return s32Ret;
    }
	...
	if ((u32LscUpdateMode == 0)&&(pstRegCfgInfo->unKey.bit1LscCfg))
	{
        /* write horizontal grid info */
        printf("Start lsc reg update in user mode !! \n");
	    for (i = 0; i < 8; i++)
	    {
...
HI_S32 ISP_RegCfgSet(ISP_DEV IspDev)
{
	...
	pstRegCfg->stRegCfg.stKernelCfg.unKey.bit1LscCfg = pstRegCfg->stRegCfg.unKey.bit1LscCfg;
	memcpy(&pstRegCfg->stRegCfg.stKernelCfg.stLscRegCfg, &pstRegCfg->stRegCfg.stLscRegCfg, sizeof(ISP_LSC_REG_CFG_S));

    //printf("lsc reg update stat = %d\n", pstRegCfg->stRegCfg.unKey.bit1LscCfg);
