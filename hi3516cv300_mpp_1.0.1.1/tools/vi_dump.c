...
HI_S32 vi_dump_save_one_raw(VIDEO_FRAME_S* pVBuf, FILE* pfd)
{
    ...
    au8Data = pUserPageAddr[0];

    /* save Y ----------------------------------------------------------------*/
    printf("saving......Raw data......u32Stride[0]: %d, width: %d\n", pVBuf->u32Stride[0], pVBuf->u32Width);
    for (h = 0; h < pVBuf->u32Height; h++)
    {
        fwrite(au8Data,pVBuf->u32Stride[0], 1,  pfd);
        au8Data += pVBuf->u32Stride[0];
		fflush(pfd);
    }
    fflush(pfd);
    printf("done u32TimeRef: %d!\n", pVBuf->u32TimeRef);
    HI_MPI_SYS_Munmap(pUserPageAddr[0], size);
...
HI_S32 SAMPLE_MISC_ViDump(VI_CHN ViChn, HI_U32 u32Cnt)
{
    ...
    snprintf(szYuvName, 128, "./vi_chn_%d_%d_%d_%s_%d.yuv", ViChn,
...
HI_S32 SAMPLE_MISC_ViDumpRaw(VI_CHN ViChn, HI_U32 u32Cnt)
{
    ...
    if (HI_MPI_VI_GetChnAttr(ViChn, &stChnAttr))
    {
		printf("HI_MPI_VI_GetChnAttr err, vi chn %d \n", ViChn);
        return HI_FAILURE;
	}
    ...
    /* make file name */
    snprintf(szYuvName, 128, "./vi_chn_%d_%d_%d_%d_16bit.raw", ViChn,
