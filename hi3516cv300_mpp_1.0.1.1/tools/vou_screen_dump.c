...
static void sample_yuv_dump(VIDEO_FRAME_S* pVBuf)
{
    unsigned int w, h;
    char* pMemContent;
    static unsigned char TmpBuff[4096];
    HI_U32 phy_addr;
...
static HI_S32 SAMPLE_MISC_VoDump(VO_LAYER VoLayer, HI_S32 s32Cnt)
{
	...
	if (s32Cnt <= 0)
    {
        printf("s32Cnt(%d) should be greater than 0!\n", s32Cnt);
        return -1;
    }
    ...
    snprintf(szYuvName, 128, "./vo(%d)_%d_%d_%s_%d.yuv", VoLayer,
...
HI_S32 vou_screen_dump(int argc, char* argv[])
#else
