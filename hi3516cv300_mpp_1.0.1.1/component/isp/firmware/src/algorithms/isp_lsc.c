...
HI_U32 GetMeshStrengthGain(HI_U16 u16Gain,HI_U16 u16MeshStrength)
{
    HI_U32 u32MeshStrengthGain;
    if(u16Gain >= 1024)
    {
    u32MeshStrengthGain = 1024 + ((HI_U32)((u16Gain-1024)*u16MeshStrength)>>12);
    }
    else
    {
        if(1024 > ((HI_U32)((1024-u16Gain)*u16MeshStrength)>>12))
        {
            u32MeshStrengthGain = 1024 - ((HI_U32)((1024-u16Gain)*u16MeshStrength)>>12);
        }
        else
        {
            u32MeshStrengthGain = 0;
        }  
    }
    return CLIP3(u32MeshStrengthGain,0,LSC_GRID_MAX_VALUE);
    
}

HI_U16 GetWeightedGain(HI_U32 u32Weight1, HI_U32 u32Weight2, HI_U32 u32Gain1,
			HI_U32 u32Gain2, HI_U32 u32DecGain, HI_U32 u32BoundUp, HI_U32 u32BoundBtm)

{
	HI_U16 u16Result;

	u16Result = (((HI_U64)u32Weight1 * u32Gain1 + (HI_U64)u32Weight2 * u32Gain2) * (HI_U64)u32DecGain)>>(10 + LSC_GRID_Q_BIT);
	u16Result = (HI_U16)(CLIP3(u16Result,u32BoundBtm,u32BoundUp));
...
HI_VOID ISP_Lsc_Fw(ISP_DEV IspDev,ISP_LSC_S *pstLsc)
{
	HI_U32 i = 0;
	HI_U32 dist[HI_LSC_GRID_LIGHT_NUM];
	HI_U32 dist_min1, dist_min2;
	HI_U32 u32Weight1,u32Weight2;
    ...
    {
		u32Weight1 = (dist_min2 * 1024)/(dist_min1 + dist_min2);
		u32Weight2 = 1024 - u32Weight1;
    }
	else
    {
		u32Weight1 = 1024;
	...
	if((lsc_light1 != g_u8PreLight[0]) || (lsc_light2 != g_u8PreLight[1]))
	{
		pstLsc->bLightChaged = HI_TRUE;
		g_u8PreLight[0] = lsc_light1;
		g_u8PreLight[1] = lsc_light2;
	}
	else if(((u32Weight1 - g_u32PreWeight[0]) > 102) || ((u32Weight2- g_u32PreWeight[1])>102))
	{
		pstLsc->bLightChaged	= HI_TRUE;
		g_u32PreWeight[0]		= u32Weight1;
		g_u32PreWeight[1]		= u32Weight2;
	}
	else
	{
	...
	if ((pstLsc->bLightChaged == HI_TRUE) || (pstLsc->bLscCoefUpdata == HI_TRUE))
	{
		...
		{
			r_gain[i] = GetWeightedGain(u32Weight1, u32Weight2, lsc_grid_info[0].au32R_Gain[i],\
						lsc_grid_info[1].au32R_Gain[i], pstLsc->noise_control[i], bound_u, bound_d);
			gr_gain[i] = GetWeightedGain(u32Weight1, u32Weight2, lsc_grid_info[0].au32Gr_Gain[i],\
						lsc_grid_info[1].au32Gr_Gain[i], pstLsc->noise_control[i], bound_u, bound_d);
			gb_gain[i] = GetWeightedGain(u32Weight1, u32Weight2, lsc_grid_info[0].au32Gb_Gain[i],\
						lsc_grid_info[1].au32Gb_Gain[i], pstLsc->noise_control[i], bound_u, bound_d);
			b_gain[i] = GetWeightedGain(u32Weight1, u32Weight2, lsc_grid_info[0].au32B_Gain[i],\
						lsc_grid_info[1].au32B_Gain[i], pstLsc->noise_control[i], bound_u, bound_d);
		}
...
static HI_VOID LscRegsInitialize(ISP_DEV IspDev)
{
	HI_S32 i;
	...
	hi_isp_lsc_cfg_enable_write(IspDev,HI_FALSE);
    ...
    enBayerFormat = hi_isp_yuv444_rggb_start_read(IspDev);
    hi_isp_lsc_rggb_cfg_write(IspDev,enBayerFormat);
	hi_isp_lsc_win_num_write(IspDev,LSC_GRID_ROWS - 2, LSC_GRID_COLS - 2);

	/* write horizontal grid info */
	for (i = 0; i < (LSC_GRID_COLS - 1)/2; i++)
	{
	    hi_isp_lsc_hwin_info_write(IspDev,i, (2*60 - 1), 68);
	}

	/* write vertical grid info */
	for (i = 0; i < (LSC_GRID_ROWS - 1)/2; i++)
	{
	    hi_isp_lsc_vwin_info_write(IspDev,i, (2*34 - 1), 120);
	}
	...
	for (i = 0; i < LSC_GRID_POINTS; i++)
	{
	    hi_isp_lsc_grr_gain_wdata_write(IspDev,HI_ISP_LSC_DEFAULT_GAINL);
	}

	hi_isp_lsc_gbb_gain_waddr_write(IspDev,0);
	for (i = 0; i < LSC_GRID_POINTS; i++)
	{
	    hi_isp_lsc_gbb_gain_wdata_write(IspDev,HI_ISP_LSC_DEFAULT_GAINL);
	}

	hi_isp_lsc_lut_update_write(IspDev,1);
...
static HI_VOID LscExtRegsInitialize(ISP_DEV IspDev)
{
	...
	memcpy(&pstLsc->stCmosLsc.stLscUniParaTable, &pstSnsDft->stLsc.stLscUniParaTable, sizeof(ISP_LSC_CABLI_UNI_TABLE_S));//grid
	...
	hi_ext_system_isp_mesh_shading_enable_write(HI_FALSE);
	hi_ext_system_isp_mesh_shading_updata_write(HI_FALSE);
	hi_ext_system_isp_mesh_shading_manu_mode_write(OP_TYPE_AUTO);
    hi_ext_system_isp_mesh_shading_meshstrength_write(HI_EXT_SYSTEM_ISP_MESH_SHADING_MESHSTRENGTH_DEFAULT);
    ...
    for ( i = 0 ; i < 8 ; i++ )
    {
        hi_ext_system_isp_mesh_shading_xgrid_write(i, pstLsc->stCmosLsc.stLscUniParaTable.u32XGrid[i]);
        hi_ext_system_isp_mesh_shading_ygrid_write(i, pstLsc->stCmosLsc.stLscUniParaTable.u32YGrid[i]);
    }
...
static HI_VOID LscReadExtRegs(ISP_DEV IspDev)
{
	...
	u16Wb_RGain = hi_isp_white_balance_gain_r_read(IspDev);
	u16Wb_GGain = (hi_isp_white_balance_gain_gr_read(IspDev) + hi_isp_white_balance_gain_gb_read(IspDev))>>1;
	u16Wb_BGain = hi_isp_white_balance_gain_b_read(IspDev);
	...
	pstLsc->bLscEnable 		= hi_ext_system_isp_mesh_shading_enable_read();
	pstLsc->bLscCoefUpdata 	= hi_ext_system_isp_mesh_shading_updata_read();
	pstLsc->enOpType		= (ISP_OP_TYPE_E)hi_ext_system_isp_mesh_shading_manu_mode_read();
    pstLsc->u16MeshStrength = hi_ext_system_isp_mesh_shading_meshstrength_read();
	for (i = 0; i < LSC_GRID_POINTS; i++)
	{
...
{	
    ...
    if ( 0 ==  hi_ext_system_isp_mesh_shading_enable_read())    
    {
		hi_isp_lsc_cfg_enable_write(IspDev,HI_FALSE);
		return HI_SUCCESS;
    }
	...
	if (OP_TYPE_MANUAL == pstLsc->enOpType)
	{
		if (pstLsc->bLscCoefUpdata == HI_TRUE)
		{
		    for (i = 0; i < LSC_GRID_POINTS; i++)
			{
                pstRegCfg->stLscRegCfg.grr_gain[i] = (GetMeshStrengthGain(pstLsc->grr_gain[i]>>13,pstLsc->u16MeshStrength)<<13)
                    + GetMeshStrengthGain(pstLsc->grr_gain[i]&0x1FFF,pstLsc->u16MeshStrength);
                pstRegCfg->stLscRegCfg.gbb_gain[i] = (GetMeshStrengthGain(pstLsc->gbb_gain[i]>>13,pstLsc->u16MeshStrength)<<13)
                    + GetMeshStrengthGain(pstLsc->gbb_gain[i]&0x1FFF,pstLsc->u16MeshStrength);
    ...
    else 
	{
		...
		{          
			if(hi_isp_wdr_enable_read(IspDev) && (pstLsc->bLscCoefUpdata == HI_TRUE))
			{
				for (i = 0; i < LSC_GRID_POINTS; i++)
				{

		   			pstRegCfg->stLscRegCfg.grr_gain[i] = HI_ISP_LSC_DEFAULT_GAINL;
					pstRegCfg->stLscRegCfg.gbb_gain[i] = HI_ISP_LSC_DEFAULT_GAINL;
			...
			else if(!hi_isp_wdr_enable_read(IspDev))
			{
				ISP_Lsc_Fw(IspDev,pstLsc);
			    if ((pstLsc->bLightChaged == HI_TRUE) || (pstLsc->bLscCoefUpdata == HI_TRUE))
				{
