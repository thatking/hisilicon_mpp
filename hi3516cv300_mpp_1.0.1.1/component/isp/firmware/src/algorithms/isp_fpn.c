...
HI_S32  ISP_SetCalibrateAttr(VI_DEV ViDev, ISP_FPN_CALIBRATE_ATTR_S *pstCalibrate)
{
    ...
    pstCalibrate->stFpnCaliFrame.u32Iso = hi_ext_system_fpn_sensor_iso_read();
...
HI_S32 ISP_SetCorrectionAttr(VI_DEV ViDev, const ISP_FPN_ATTR_S *pstCorrection)
{
    ...
    hi_ext_system_manual_fpn_opmode_write(pstCorrection->enOpType);


    hi_ext_system_manual_fpn_strength_write(pstCorrection->stManual.u32Strength);

    if(OP_TYPE_AUTO == pstCorrection->enOpType)
    {
        ISP_GET_CTX(0, pstIspCtx);   
        //u32Iso = pstIspCtx->stLinkage.u32SensorIso;
        u32Iso = hi_ext_system_fpn_sensor_iso_read();
        hi_ext_system_manual_fpn_update_write(0x1);
        hi_ext_system_manual_fpn_iso_write(pstCorrection->stFpnFrmInfo.u32Iso);
		...
		{
		    hi_isp_fpn_coverflowthr_write(ispDev, FPN_OVERFLOWTHR);
		}
		else
		{
			hi_isp_fpn_coverflowthr_write(ispDev, FPN_OVERFLOWTHR_OFF);
		}
	...
	hi_ext_system_fpn_strength_write(u32Gain);
    ...
    if(HI_SUCCESS == s32Ret)
    {
        ...
        hi_ext_system_manual_fpn_CorrCfg_write(HI_TRUE);
    }
    else
    {
       
        hi_ext_system_manual_fpn_CorrCfg_write(HI_FALSE);
    }
...
HI_S32 ISP_GetCorrectionAttr(VI_DEV ViDev, ISP_FPN_ATTR_S *pstCorrection)
{
    ...
    if (OP_TYPE_MANUAL == pstCorrection->enOpType)
    {
        pstCorrection->stManual.u32Strength = hi_ext_system_manual_fpn_strength_read();
...
{
    hi_ext_system_fpn_sensor_iso_write(HI_EXT_SYSTEM_FPN_SENSOR_ISO_DEFAULT);
	hi_ext_system_manual_fpn_iso_write(HI_EXT_SYSTEM_FPN_MANU_ISO_DEFAULT);
    return;
}
...
{
    g_s32Iso = 0;  
    hi_ext_system_manual_fpn_CorrCfg_write(HI_EXT_SYSTEM_FPN_MANU_CORRCFG_DEFAULT);
    hi_ext_system_manual_fpn_strength_write(HI_EXT_SYSTEM_FPN_STRENGTH_DEFAULT);
    hi_ext_system_manual_fpn_opmode_write(HI_EXT_SYSTEM_FPN_OPMODE_DEFAULT);
    hi_ext_system_manual_fpn_update_write(HI_EXT_SYSTEM_FPN_MANU_UPDATE_DEFAULT);
    return;
}
...
static HI_VOID FPNRegsInitialize(ISP_DEV IspDev)
{
	...
	hi_isp_fpn_width_write(IspDev, u32Width-1);
	hi_isp_fpn_height_write(IspDev, u32Height-1);
...
HI_S32 FPNUpdateExtRegs(ISP_DEV IspDev)
{
    ...
    hi_ext_system_fpn_sensor_iso_write(pstIspCtx->stLinkage.u32SensorIso);
...
HI_VOID IspSetStrength(ISP_DEV IspDev)
{
    ...
    u8FpnOpMode = hi_ext_system_manual_fpn_opmode_read();    
    ...
    //u32Iso = pstIspCtx->stLinkage.u32SensorIso;
    u32Iso = hi_ext_system_fpn_sensor_iso_read();
	u8FpnEn = hi_isp_fpn_enable_read(IspDev);
    ...
    if(u8FpnEn && ((g_s32Iso != u32Iso) || hi_ext_system_manual_fpn_update_read()))
    {     	
        if(hi_ext_system_manual_fpn_update_read())
        {
            hi_ext_system_manual_fpn_update_write(0);
        }


        u32CalibrateIso = hi_ext_system_manual_fpn_iso_read();        
        u32Gain = ISP_Fpn_GetStrength(u32Iso, 0 , 255, 0, u32CalibrateIso);        
        u32Gain = ISP_FPN_CLIP(u32Gain,0 ,1023);        
        for (i = 0; i < FPN_CHN_NUM; i++)
        {
            u32GainOffset[i] = hi_isp_fpn_corr_gainoffset_read(IspDev, i);   
            Offfset  = u32GainOffset[i] & 0xFFF;
            u32GainOffset[i] = (u32Gain << 16) + Offfset;             
            hi_isp_fpn_corr_gainoffset_write(IspDev, i, u32GainOffset[i]); 
        }  
...
HI_S32 FpnProcWrite(ISP_DEV IspDev, ISP_CTRL_PROC_WRITE_S *pstProc)
{
    ...
    u32GainOffset = hi_isp_fpn_corr_gainoffset_read(IspDev, 0);
	u32Strength = hi_isp_ext_system_fpn_strength_read();
    
    if(hi_isp_fpn_enable_read(IspDev))
    {
        ISP_PROC_PRINTF(&stProcTmp, pstProc->u32WriteLen,
        "%8d" "%4d"  "%9d"  "%8d",
        hi_isp_fpn_enable_read(IspDev),
        hi_ext_system_manual_fpn_opmode_read(),
        u32Strength,
...
static __inline HI_S32 FPNImageResWrite(ISP_DEV IspDev, ISP_CMOS_SENSOR_IMAGE_MODE_S *pstRes)
{

	hi_isp_fpn_width_write(IspDev, pstRes->u16Width-1);
	hi_isp_fpn_height_write(IspDev,  pstRes->u16Height-1);
...
HI_S32 ISP_FPNCtrl(ISP_DEV IspDev, HI_U32 u32Cmd, HI_VOID *pValue)
{
    switch (u32Cmd)
    {
        ...
        case ISP_CHANGE_IMAGE_MODE_SET:
            FPNImageResWrite(IspDev,(ISP_CMOS_SENSOR_IMAGE_MODE_S *)pValue);
            break;			
