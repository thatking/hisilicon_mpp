...
static HI_VOID CompanderRegsInitialize(ISP_DEV IspDev)
{
	...
	hi_isp_compander_enable_write(IspDev,HI_FALSE);	
	...
	hi_isp_compander_waddr_write(IspDev,0);

	for (_i = 0; _i < COMPANDER_BITSOUT_XMAX; _i++)
	{
		hi_isp_compander_wdata_write(IspDev,pstCompander->u32Compander_LUT[_i]);
	}	
    
    	ISP_SensorGetBlc(IspDev, &pstSnsBlackLevel);

	hi_isp_compander_bitw_in_write    (IspDev,pstCompander->u32BitDepthIn);
	hi_isp_compander_bitw_out_write   (IspDev,pstCompander->u32BitDepthOut);
	...
	{             
		hi_isp_compander_enable_write(IspDev,HI_TRUE);		
	}
	else 
	{                    
		hi_isp_compander_enable_write(IspDev,HI_FALSE);		
