...
HI_S32 ISP_ProcInit(ISP_DEV IspDev)
{
    ...
    pstProc->stProcMem.pProcVirtAddr = HI_MPI_SYS_MmapCache(pstProc->stProcMem.u32ProcPhyAddr,
...
freeproc:
    u32Ret = ioctl(g_as32IspFd[0], ISP_PROC_EXIT);
	if(HI_SUCCESS != u32Ret)
	{
      printf("Isp Proc exit failed\r\n");
	}
    return s32Ret;
}
...
HI_S32 ISP_ProcWrite(const ISP_ALG_NODE_S *astAlgs, ISP_DEV IspDev)
{
    ...
    u32Ret = ioctl(g_as32IspFd[0], ISP_PROC_WRITE_OK);
	if(0!= u32Ret)
	{
       printf("Isp proc write  failed!\n");
	}
