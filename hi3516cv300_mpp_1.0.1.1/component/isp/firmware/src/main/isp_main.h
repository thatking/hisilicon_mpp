...
do{\
    ...
    {\
        ...
        {\
            ISP_TRACE(HI_DBG_ERR, "ISP[%d] get Mem info failed in %s!\n", dev, __FUNCTION__);\
            return HI_ERR_ISP_MEM_NOT_INIT;\
        }\
        if (g_astIspCtx[dev].bMemInit != HI_TRUE)\
        {\
            ISP_TRACE(HI_DBG_ERR, "ISP[%d] Mem NOT Init %d in %s!\n", dev, g_astIspCtx[dev].bMemInit, __FUNCTION__);\
            return HI_ERR_ISP_MEM_NOT_INIT;\
        }\
    }\
}while(0)
...
do{\
    ...
    {\
        ISP_TRACE(HI_DBG_ERR, "Isp[%d] WDR mode doesn't config in %s!\n", dev, __FUNCTION__);\
...
do{\
    ...
    {\
        ISP_TRACE(HI_DBG_ERR, "Isp[%d] Pub Attr doesn't config in %s!\n", dev, __FUNCTION__);\
        return HI_ERR_ISP_ATTR_NOT_CFG;\
    }\
}while(0)
...
do{\
    ...
    {\
        ISP_TRACE(HI_DBG_ERR, "Isp[%d] doesn't initialized in %s!\n", dev, __FUNCTION__);\
        return HI_ERR_ISP_ATTR_NOT_CFG;\
    }\
}while(0)
