...
{
    if ((ISP_REG_BASE != (u32BaseAddr & 0xffff0000)) && ((ISP_REG_BASE+0x20000) != (u32BaseAddr & 0xffff0000)) \
		&& ((ISP_REG_BASE+0x30000) != (u32BaseAddr & 0xffff0000)))
    {
        return 0;
    }
...
static inline HI_VREG_ADDR_S *VReg_Match(HI_U32 u32BaseAddr)
{
    ...
    {
        case ISP_REG_BASE :
		case ISP_REG_BASE + 0x20000:
		case ISP_REG_BASE + 0x30000:
...
HI_U32 VReg_GetVirtAddr(HI_U32 u32BaseAddr)
{
    ...
    /* map virtaddr */
    u32Size = (((ISP_REG_BASE == (u32BaseAddr & 0xffff0000)) || ((ISP_REG_BASE+0x20000) == (u32BaseAddr & 0xffff0000))\
    			|| ((ISP_REG_BASE+0x30000) == (u32BaseAddr & 0xffff0000))) ? ISP_REG_SIZE :
...
HI_U32 IO_READ16_VI(HI_U32 u32Addr)
{
    ...
    {
        u32Value = IO_READ32_VI( u32Addr );
        u32Value = (u32Value & 0xFFFF);
    }
    else
    {
        u32Value = IO_READ32_VI(((u32Addr>>REG_ACCESS_WIDTH_1) - 2)<<REG_ACCESS_WIDTH_1);
...
HI_S32 IO_WRITE16_VI(HI_U32 u32Addr, HI_U32 u32Value)
{
    ...
    if(((u32Addr>>REG_ACCESS_WIDTH_1) & 2 )== 0)
    {
        u32Current = IO_READ32_VI(u32Addr);
        IO_WRITE32_VI(u32Addr, (u32Value & 0x0000FFFF )|(u32Current & 0xFFFF0000 ));
    }
    else
    {
        u32TmpAddr = ((u32Addr >>REG_ACCESS_WIDTH_1)-2)<<REG_ACCESS_WIDTH_1;
        u32Current = IO_READ32_VI(u32TmpAddr);
        IO_WRITE32_VI(u32TmpAddr, ((u32Value & 0x0000FFFF ) << 16)|(u32Current & 0x0000FFFF ));
    }
...
HI_U32 IO_READ32(HI_U32 u32Addr)
{
	...
	else if (((ISP_REG_BASE+0x20000) == (u32Addr & 0xFFFF0000)) || ((ISP_REG_BASE+0x30000) == (u32Addr & 0xFFFF0000)))
	{
	    pu32Addr = (HI_U32 *)((HI_U32)u32VirtAddrBase + ((u32Addr - ISP_REG_BASE) & 0x3ffff));
	}
    else
    {
    ...
    if(((HI_U32)pu32Addr) % 4 != 0)
    {
        HI_U8* p1 = (HI_U8*)&u32Value;
        HI_U8* p2 = (HI_U8*)pu32Addr;
        *p1++ = *p2++;
        *p1++ = *p2++;
        *p1++ = *p2++;
        *p1++ = *p2++;
...
HI_S32 IO_WRITE32(HI_U32 u32Addr, HI_U32 u32Value)
{
	...
	if ((ISP_REG_BASE == (u32Addr & 0xFFFF0000)) || (ISP_VREG_BASE == (u32Addr & 0xFFFF0000)))
	{
		pu32Addr = (HI_U32 *)((HI_U32)u32VirtAddrBase + (u32Addr & 0xffff));
	}
	else if (((ISP_REG_BASE+0x20000) == (u32Addr & 0xFFFF0000)) || ((ISP_REG_BASE+0x30000) == (u32Addr & 0xFFFF0000)))
    ...
    if(((HI_U32)pu32Addr) % 4 != 0)
    {
    HI_U8* p1 = (HI_U8*)pu32Addr;
    HI_U8* p2 = (HI_U8*)&u32Value;
    *p1++ = *p2++;
    *p1++ = *p2++;
    *p1++ = *p2++;
    *p1++ = *p2++;
