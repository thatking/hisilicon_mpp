...
HI_S32 SAMPLE_COMM_AUDIO_StartAi(AUDIO_DEV AiDevId, HI_S32 s32AiChnCnt,
        ...
        AI_VQE_CONFIG_S *pstAiVqeCfg, AI_VQE3_CONFIG_S *pstAiVqe3Cfg)
{
    HI_S32 i, s32Ret;
    ...
    for (i=0; i<s32AiChnCnt; i++)
    {
        ...
        if (NULL != pstAiVqeCfg)
        {
            s32Ret = HI_MPI_AI_SetVqeAttr(AiDevId, i, AiDevId, i, pstAiVqeCfg);
            if (s32Ret)
            {
        ...
        if (NULL != pstAiVqe3Cfg)
        {
            s32Ret = HI_MPI_AI_SetVqe3Attr(AiDevId, i, AiDevId, i, pstAiVqe3Cfg);
            if (s32Ret)
            {
                printf("%s: HI_MPI_AI_SetVqe3Attr(%d,%d) failed with %#x\n", __FUNCTION__, AiDevId, i, s32Ret);
                return s32Ret;
            }
...
HI_S32 SAMPLE_COMM_AUDIO_StartAiEx(AUDIO_DEV AiDevId, HI_S32 s32AiChnCnt,
        AIO_ATTR_S *pstAioAttr, HI_BOOL bAnrEn, AUDIO_RESAMPLE_ATTR_EX_S *pstAiReSmpAttr,
        AI_VQE_CONFIG_S *pstAiVqeCfg, AI_VQE3_CONFIG_S *pstAiVqe3Cfg)
{
    HI_S32 i, s32Ret;
    ...
    for (i=0; i<s32AiChnCnt; i++)
    {
        ...
        if (NULL != pstAiReSmpAttr)
        {
            if (HI_MPI_AI_EnableReSmpEx(AiDevId, i, pstAiReSmpAttr))
            {
                printf("%s: HI_MPI_AI_EnableReSmp(%d,%d) failed with %#x\n",\
                        __FUNCTION__, AiDevId, i, s32Ret);
                return HI_FAILURE;
...
HI_S32 SAMPLE_COMM_AUDIO_StopAiEx(AUDIO_DEV AiDevId, HI_S32 s32AiChnCnt,
        HI_BOOL bAnrEn, HI_BOOL bResampleEn)
{
    HI_S32 i;
    for (i=0; i<s32AiChnCnt; i++)
    {
        if (HI_TRUE == bResampleEn)
        {
            HI_MPI_AI_DisableReSmpEx(AiDevId, i);
        }
        if (HI_TRUE == bAnrEn)
        {
            HI_MPI_AI_DisableAnr(AiDevId, i);
...
HI_S32 SAMPLE_COMM_AUDIO_StartAo(AUDIO_DEV AoDevId, AO_CHN AoChn,
        AIO_ATTR_S *pstAioAttr, AUDIO_RESAMPLE_ATTR_S *pstAoReSmpAttr, AO_VQE3_CONFIG_S *pstAoVqe3Cfg)
{
    HI_S32 s32Ret;
...
HI_S32 SAMPLE_COMM_AUDIO_StopAoEx(AUDIO_DEV AoDevId, AO_CHN AoChn, HI_BOOL bResampleEn)
{
    if (HI_TRUE == bResampleEn)
    {
        HI_MPI_AO_DisableReSmpEx(AoDevId, AoChn);
    }
    HI_MPI_AO_DisableChn(AoDevId, AoChn);
    HI_MPI_AO_Disable(AoDevId);
    return HI_SUCCESS;
