...
static HI_S32 SAMPLE_RGN_SYS_Init(HI_VOID)
{
    ...
    struVbConf.astCommPool[0].u32BlkSize = 2592 * 1944 * 2;
...
HI_S32 SAMPLE_RGN_StartVpss(HI_S32 s32VpssGrpNum, HI_U32 u32VpssChn)
{
    ...
    for (i = 0; i < s32VpssGrpNum; i++)
    {
        ...
        u32CoverMask = 255;
        s32Ret = HI_MPI_VPSS_SetChnCover(i, u32VpssChn, u32CoverMask);
        if (HI_SUCCESS != s32Ret)
        {
            printf("HI_MPI_VPSS_SetChnCover fail! Grp: %d, Chn: %d! s32Ret: 0x%x.\n", i, u32VpssChn, s32Ret);
            return s32Ret;
        }
...
HI_VOID* SAMPLE_RGN_VencGetStream(void* pData)
{
    ...
    snprintf(acmd, 128, "[[ -e jpeg%d ]] || mkdir jpeg%d", pstVencThread->VeChnId, pstVencThread->VeChnId);
    system(acmd);
    ...
    while (HI_FALSE == bExit || i < pstVencThread->s32FrmCnt)
    {
        ...
        if (stChnStat.u32CurPacks > 0 || stChnStat.u32LeftStreamBytes > 0)
        {
            if (bTravel)
            {
                char pfilename[FILE_NAME_LEN];
                snprintf(pfilename, FILE_NAME_LEN, "./jpeg%d/stream_fff_009_%d.jpeg", pstVencThread->VeChnId, i);
            ...
            else if (bShape1)
            {
                ...
                { s32Tmp = 14; }
                snprintf(pfilename, FILE_NAME_LEN, "./jpeg%d/stream_fff_0%d_%d.jpeg", pstVencThread->VeChnId, s32Tmp, i);
                pastream = fopen( pfilename, "wb" );
                HI_ASSERT( NULL != pastream);
...
HI_S32 SAMPLE_RGN_AddOsdToVencAndVpss(HI_VOID)
{
    ...
    stVpssOsdReverseInfo.VpssChn                 = VPSS_CHN2;
     ...
     *************************************************/
    char pfilename[FILE_NAME_LEN];
    snprintf(pfilename, FILE_NAME_LEN, "venc_encode_stream.h264");
    pastream = fopen(pfilename, "wb");
    HI_ASSERT( NULL != pastream);
