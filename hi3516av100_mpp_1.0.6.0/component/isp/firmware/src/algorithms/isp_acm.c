...
static inline HI_S32 ACM_CheckAttr(ISP_ACM_ATTR_S *pstAcmAttr)
{
    ...
    if(pstAcmAttr->enMode < ISP_ACM_MODE_BLUE || pstAcmAttr->enMode >= ISP_ACM_MODE_BUTT  )
    {
...
HI_S32 ISP_ACM_GetCoeff(ISP_ACM_LUT_S *pstAcmLUT,ISP_ACM_MODE_E enMode)
{
    ...
    ISP_CHECK_OPEN(0);    
    if(enMode < ISP_ACM_MODE_BLUE || enMode >= ISP_ACM_MODE_BUTT  )
    {
        ISP_TRACE(HI_DBG_ERR, "Error acm attribute error enMode:%d\n",enMode);
        return HI_DEF_ERR(HI_ID_ISP,EN_ERR_LEVEL_ERROR,EN_ERR_ILLEGAL_PARAM);
    }
...
HI_S32 AcmProcWrite(ISP_DEV IspDev, ISP_CTRL_PROC_WRITE_S *pstProc)
{
    ...
    if(hi_vi_acm_read_enable())
    {
        ISP_PROC_PRINTF(&stProcTmp, pstProc->u32WriteLen,
        "%8d" "%7d"     "%9d"    "%8d"     "%8d"   "%8d"   "%8d"      "%8d"       "%8d"      "%8d",
        hi_vi_acm_read_enable(),
        hi_vi_acm_read_dbg_enable(),
        pstAcmAttr->enMode,
        hi_vi_acm_read_stretch(),
        hi_vi_acm_read_cliprange(),
        hi_vi_acm_read_cliporwrap(),
        hi_vi_acm_read_cbcrth(),
        hi_vi_acm_read_luma_gain(),
        hi_vi_acm_read_hue_gain(),
        hi_vi_acm_read_sat_gain()
        );
    }
    else
    {
