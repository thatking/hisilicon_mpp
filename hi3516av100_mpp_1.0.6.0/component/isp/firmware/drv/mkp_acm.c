...
/*****************************************************************************
 ...
 Description  : 将驱动传递进来的系数加载到申请的内存中
 Input        :   
 Output       : none
...
HI_VOID ACM_DRV_UpdateRegister(HI_U32 u32AcmCoefAddr)
{
    ...
    //hi_vi_acm_write_dbg_enable(HI_FALSE);
    hi_vi_acm_write_luma_gain(pstAcmCtx->stAcmCtrl.u32GainLuma);
    hi_vi_acm_write_sat_gain(pstAcmCtx->stAcmCtrl.u32GainSat);
    hi_vi_acm_write_hue_gain(pstAcmCtx->stAcmCtrl.u32GainHue);
    hi_vi_acm_write_cbcrth(pstAcmCtx->stAcmCtrl.u32CbcrThr);
    hi_vi_acm_write_cliprange(pstAcmCtx->stAcmCtrl.u32ClipRange);
    hi_vi_acm_write_stretch(pstAcmCtx->stAcmCtrl.u32Stretch);
    hi_vi_acm_write_cliporwrap(pstAcmCtx->stAcmCtrl.u32AcmClipOrWrap);
    
    hi_vi_acm_write_coeff_addr(u32AcmCoefAddr);
    /* In chip power up, there maybe a wrong interrput!!! */
    hi_vi_acm_write_para_up(1);

    hi_vi_acm_write_enable((HI_BOOL)pstAcmCtx->stAcmCtrl.bEnable);
    hi_vi_acm_write_dbg_enable((HI_BOOL)pstAcmCtx->stAcmCtrl.bDemoEnable);
