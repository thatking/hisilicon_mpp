...
HI_S32 SAMPLE_AUDIO_AiAenc(AIO_ATTR_S *pstAioAttr)
{
    ...
    ********************************************/
    ...
    /*enable vqe*/
	{
    	...
    	AI_HPF_ATTR_S   stHpfAttr;

    	stAiVqeAttr.s32SampleRate       = AUDIO_SAMPLE_RATE_8000;
    	stAiVqeAttr.s32FrameSample      = SAMPLE_AUDIO_PTNUMPERFRM;
    	stAiVqeAttr.bAecOpen            = HI_FALSE;
    	stAiVqeAttr.stAecCfg.enAecMode  = 0;
    	stAiVqeAttr.bAlcOpen            = HI_TRUE;
    	stAiVqeAttr.stAlcCfg.s32MinLev  = -16;
    	stAiVqeAttr.stAlcCfg.s32MaxLev  = -4;
    	stAiVqeAttr.stAlcCfg.u32MaxGain = 6;
    	stAiVqeAttr.bAnrOpen            = HI_TRUE;

        s32Ret = HI_MPI_AI_GetHpfAttr(AiDev, AiChn, &stHpfAttr);
        if (HI_SUCCESS != s32Ret)
        {
            SAMPLE_DBG(s32Ret);
            return HI_FAILURE;
        }
        stHpfAttr.bHpfOpen = HI_TRUE;
        stHpfAttr.stHpfCfg.enHpfFreq = AUDIO_HPF_FREQ_150;
        s32Ret = HI_MPI_AI_SetHpfAttr(AiDev, AiChn, &stHpfAttr);
        if (HI_SUCCESS != s32Ret)
        {
            SAMPLE_DBG(s32Ret);
            return HI_FAILURE;
         ...
         s32Ret = HI_MPI_AI_SetVqeAttr(AiDev, AiChn, AoDev, AoChn, &stAiVqeAttr);
        if (HI_SUCCESS != s32Ret)
        {
            SAMPLE_DBG(s32Ret);
            return HI_FAILURE;
        }
        s32Ret = HI_MPI_AI_EnableVqe(AiDev, AiChn);
        if (HI_SUCCESS != s32Ret)
        {
            SAMPLE_DBG(s32Ret);
            return HI_FAILURE;
