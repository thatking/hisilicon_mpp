...
void lcd_ili9342c_init_horizontal_all(void)
{
    ...
    ssp_write_dat(0xEC);//40,42,60,62,cc。C0客人反映OK，麻点是PCLK的时序没有配合好,如果客人是上升沿的，那么我的为下降沿.
    ...
    ssp_write_cmd(0xB5);//此寄存器可以修改LCD接收数据的第一位是什么，RGB可选
    ssp_write_dat(0x02);
    ssp_write_dat(0x02);
    ssp_write_dat(0x0a);
    ssp_write_dat(0x12);//HBP，这一位可以选择RGB输入的是哪一位
