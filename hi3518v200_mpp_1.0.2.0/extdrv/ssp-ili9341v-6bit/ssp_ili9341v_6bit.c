...
void lcd_ili9341_init_vertical_serial(void)
{
    ...
    ssp_write_cmd(0xED);  //power on sequence control，修改默认值后没有明显效果
    ssp_write_dat(0x64);
    ssp_write_dat(0x03);
    ssp_write_dat(0X12);
    ssp_write_dat(0X81);
    ...
    ssp_write_cmd(0x36);    // Memory Access Control  横竖屏  可以修改RGB/BGR
    ssp_write_dat(0x08);
    ...
    ssp_write_dat(0x00);
    //    ssp_write_dat(0x1B);
    //ssp_write_dat(0x1C);   //68帧
    ssp_write_dat(0x12);     //106帧

    ssp_write_cmd(0xB6);    // Display Function Control ，可以改变屏的左右显示
    ssp_write_dat(0x0A);
    ssp_write_dat(0xA2);
